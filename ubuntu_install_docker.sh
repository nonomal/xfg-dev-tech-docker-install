...
curl -fsSL https://download.docker.com/linux/ubuntu/gpg | gpg --dearmor -o /etc/apt/keyrings/docker.gpg || error "添加Docker GPG密钥失败"
...
echo \
  ...
  tee /etc/apt/sources.list.d/docker.list > /dev/null || error "设置Docker仓库失败"
...
# 安装Docker CE
info "安装Docker CE..."
apt-get install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin || error "Docker安装失败"
