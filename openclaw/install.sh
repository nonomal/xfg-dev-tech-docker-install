...
install_openclaw_npm() {
    ...
    local log
    log="$(mktempfile)"
    if ! SHARP_IGNORE_GLOBAL_LIBVIPS="$SHARP_IGNORE_GLOBAL_LIBVIPS" npm --registry=https://registry.npmmirror.com --loglevel "$NPM_LOGLEVEL" ${NPM_SILENT_FLAG:+$NPM_SILENT_FLAG} --no-fund --no-audit install -g "$spec" 2>&1 | tee "$log"; then
        if grep -q "ENOTEMPTY: directory not empty, rename .*openclaw" "$log"; then
            ...
            cleanup_npm_openclaw_paths
            SHARP_IGNORE_GLOBAL_LIBVIPS="$SHARP_IGNORE_GLOBAL_LIBVIPS" npm --registry=https://registry.npmmirror.com --loglevel "$NPM_LOGLEVEL" ${NPM_SILENT_FLAG:+$NPM_SILENT_FLAG} --no-fund --no-audit install -g "$spec"
            return $?
        fi
...
install_node() {
    if [[ "$OS" == "macos" ]]; then
        echo -e "${WARN}→${NC} Installing Node.js via n..."
        fix_npm_permissions
        ...
        # Add N_PREFIX to shell rc files
        local n_path_line='export N_PREFIX="$HOME/.n"'
        local path_line='export PATH="$N_PREFIX/bin:$PATH"'
        for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
            if [[ -f "$rc" ]]; then
                 if ! grep -q "N_PREFIX" "$rc"; then
                    echo "$n_path_line" >> "$rc"
                 fi
                 if ! grep -q "$N_PREFIX/bin" "$rc"; then
                    echo "$path_line" >> "$rc"
                 fi
            fi
        done
        ...
        n 22
        echo -e "${SUCCESS}✓${NC} Node.js installed via n"
    elif [[ "$OS" == "linux" ]]; then
...
ensure_pnpm() {
    ...
    fix_npm_permissions
    npm install --registry=https://registry.npmmirror.com -g pnpm@10
    echo -e "${SUCCESS}✓${NC} pnpm installed"
    return 0
}
...
install_openclaw_from_git() {
    ...
    SHARP_IGNORE_GLOBAL_LIBVIPS="$SHARP_IGNORE_GLOBAL_LIBVIPS" pnpm -C "$repo_dir" install --registry=https://registry.npmmirror.com
...
install_openclaw_manual_no_git() {
    local version="$1"
    echo -e "${WARN}→${NC} Git is unavailable. Attempting manual install from tarball (removing git dependencies)..."
    
    local workdir
    workdir="$(mktemp -d)"
    pushd "$workdir" >/dev/null || return 1
    
    echo -e "${WARN}→${NC} Downloading openclaw@$version..."
    if [[ "$version" == "2026.1.29" ]]; then
        local url="https://gitcode.com/Yao__Shun__Yu/xfg-dev-tech-docker-install/releases/download/openclaw%2F2026.1.29/openclaw-2026.1.29.tgz"
        if ! download_file "$url" "openclaw-2026.1.29.tgz"; then
            echo -e "${ERROR}✗${NC} Failed to download openclaw package from gitcode"
            popd >/dev/null || true
            rm -rf "$workdir"
            return 1
        fi
    elif ! npm pack "openclaw@$version" --registry=https://registry.npmmirror.com; then
        echo -e "${ERROR}✗${NC} Failed to download openclaw package"
        popd >/dev/null || true
        rm -rf "$workdir"
        return 1
    fi
    ...
    echo -e "${WARN}→${NC} Creating mock for @whiskeysockets/baileys..."
    mkdir -p node_modules/@whiskeysockets/baileys
    cat > node_modules/@whiskeysockets/baileys/package.json <<EOF
{
...
EOF
    cat > node_modules/@whiskeysockets/baileys/index.js <<EOF
export default {};
export const makeWASocket = () => ({ ev: { on: () => {}, off: () => {} }, ws: { on: () => {} } });
export const DisconnectReason = { loggedOut: "loggedOut" };
export const useMultiFileAuthState = async () => ({ state: { creds: {}, keys: {} }, saveCreds: async () => {} });
export const Browsers = { ubuntu: () => {} };
...
export const normalizeMessageContent = (msg) => msg ?? null;
export const extractMessageContent = (msg) => msg ?? null;
export const getContentType = () => null;
EOF

    echo -e "${WARN}→${NC} Bundling mock dependency..."
    python3 -c "import json; d=json.load(open('package.json')); d.setdefault('bundledDependencies', []).append('@whiskeysockets/baileys'); json.dump(d, open('package.json','w'))"
    
    echo -e "${WARN}→${NC} Repacking modified package..."
    local modified_tarball
    if ! modified_tarball=$(npm pack . --quiet --ignore-scripts); then
         echo -e "${ERROR}✗${NC} Repack failed"
         popd >/dev/null || true
         rm -rf "$workdir"
         return 1
    fi
    
    echo -e "${WARN}→${NC} Installing modified package from tarball..."
    if npm install -g "$modified_tarball" --registry=https://registry.npmmirror.com; then
         echo -e "${SUCCESS}✓${NC} OpenClaw installed (limited functionality)"
         popd >/dev/null || true
         rm -rf "$workdir"
         return 0
    else
         echo -e "${ERROR}✗${NC} Manual install failed"
         popd >/dev/null || true
         rm -rf "$workdir"
         return 1
    fi
...
install_openclaw() {
    ...
    if ! install_openclaw_npm "${install_spec}"; then
        ...
        if ! git --version >/dev/null 2>&1 || [[ "$(git --version 2>&1)" == *"xcrun: error"* ]]; then
             if install_openclaw_manual_no_git "$resolved_version"; then
                 return 0
             fi
        fi
